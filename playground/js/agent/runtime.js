...
const ITERATION_LIMIT_WRAP_UP =
  "反復上限に達し、これ以上ツールは使えません。ここまでに生成した地図コード・確認できたデータ・残っている問題を簡潔にまとめ、未完了なら残りの手順も示してください。";
 ...
 * inspect_dataは既に要約済みだが、他ツールも含めて一律で上限を保証する。
 */
function capToolResultText(text) {
  if (text.length <= TOOL_RESULT_CHAR_CAP) return text;
  return `${text.slice(0, TOOL_RESULT_CHAR_CAP)}…（結果が大きいため省略しました。inspect_data等で必要な詳細だけ取得してください）`;
}
