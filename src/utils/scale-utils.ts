 ...
 * ```
 */
export function createProportionalScale(
  values: number[],
  options: ProportionalScaleOptions = {}
): (value: number) => number {
  const finite = values.filter((v) => typeof v === 'number' && isFinite(v));
  if (finite.length === 0) {
    throw new Error('[thematika] createProportionalScale: 有効な数値が1つもありません。');
  }

  const minValue = options.minValue ?? 0;
  const maxValue = options.maxValue ?? Math.max(...finite);
  const minRadius = options.minRadius ?? 1;
  const maxRadius = options.maxRadius ?? 30;

  if (maxValue <= minValue) {
    throw new Error('[thematika] createProportionalScale: maxValueはminValueより大きい必要があります。');
  }
  ...
  return (value: number): number => {
    ...
    return minRadius + Math.sqrt(t) * radiusRange;
  };
}
